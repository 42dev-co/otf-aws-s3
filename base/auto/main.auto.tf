...
locals {
  bucket_files = [for f in fileset(path.module, "./resources/**/*.yaml"): [try(yamldecode(file(f)), null), f]]
  bucket_files_with_err = [ for f in local.bucket_files: f if f[0] == null ]
  bucket_files_has_err = length(local.bucket_files_with_err) > 0
  buckets = local.bucket_files_has_err ? {} : { for k, v in merge([for t in local.bucket_files: t[0]]...): "${k}.${var.group}.${var.account}.${var.domain}" => merge(v, { policy_key = k})  if try(v.create, true) }
...
module "s3_bucket" {
  ...
  policy              = try(local.policies["resources/policies/${each.value.policy_key}.json"], null)
  ...
  attach_policy                              = try(length(local.policies["resources/policies/${each.value.policy_key}.json"]) > 0, false) ? true : false
...
output "errors" {
  value = local.bucket_files_with_err
}